...
class LogisticRegression():
    ...
    def __init__(self, dim=2, eta=0.1, momentum=0.9, iter=100):
        self.dim = dim
        self.eta = eta
        self.momentum = momentum
        self.iter = iter
        self.pv = np.array([0 for i in range(0, self.dim + 1)])
    ...
    def optimisation(self, sample):
        ...
        v = [(self.momentum * pv_i) + diff_i for pv_i, diff_i in zip(self.pv, diff)]
        self.w = [w_i - v_i for w_i, v_i in zip(self.w, v)]
        self.pv = np.array(v)
